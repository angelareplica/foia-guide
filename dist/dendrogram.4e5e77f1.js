...
var width = 736 - margin.left - margin.right;
...
var div = d3.select('body').append('div').attr('class', 'tooltip-dend').style('opacity', 0);

// Add the links between nodes:
svg.selectAll('path').data(root.descendants().slice(1)).enter().append('path').attr("d", function (d) {
  return "M" + d.y + "," + d.x + "C" + (d.parent.y + 50) + "," + d.x + " " + (d.parent.y + 150) + "," + d.parent.x // 50 and 150 are coordinates of inflexion, play with it to change links shape
  + " " + d.parent.y + "," + d.parent.x;
}).style("fill", 'none')
// .attr("stroke", '#C6CCC1')
.attr("stroke", '#8ec1bc').attr('stroke-width', '3').style('opacity', .5).lower();

// add a circle for each node
svg.selectAll("g").data(root.descendants()).enter().append("g").attr("transform", function (d) {
  return "translate(" + d.y + "," + d.x + ")";
}).append("circle").attr("r", 9).style("fill", "#B8A1A8").attr("stroke", "black").style("stroke-width", 1).on('mouseover', function (d) {
  // console.log(d)

  // make the bar chart highlight on mouseover
  d3.select(this).transition().duration(100).attr('r', 11).style('fill', '#BB1427');

  // make tooltip visible
  div.transition().duration(100).style('opacity', 1);

  div.html(d.data.description).style('left', d3.event.pageX - 70 + 'px')
  // .style('right', d3.event.pageX + 'px')
  .style('top', d3.event.pageY + 15 + 'px');
}).on('mouseout', function (d) {
  d3.select(this).transition().duration(100).attr('r', 9).style('fill', '#B8A1A8');
...
svg.selectAll('rect').data(root.descendants()).enter().append("rect").attr('class', 'dend-labels-rects').attr("transform", function (d) {
...
// .attr('fill', '#FFD67C')
.attr('fill', '#8ec1bc').style('opacity', 0.6);

// add step labels
svg.selectAll('text').data(root.descendants()).enter().append("text").attr('class', 'dend-labels').attr("transform", function (d) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","dendrogram.js"], null)
//# sourceMappingURL=/dendrogram.4e5e77f1.map