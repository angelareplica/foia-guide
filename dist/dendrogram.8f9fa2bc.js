...
cluster(root); // add a div for the tooltip

var div = d3.select('body').append('div').attr('class', 'tooltip-dend').style('opacity', 0); // Add the links between nodes:
...
}).style("fill", 'none') // .attr("stroke", '#C6CCC1')
.attr("stroke", '#8ec1bc').attr('stroke-width', '3').style('opacity', .5).lower(); // add a circle for each node
...
svg.selectAll("g").data(root.descendants()).enter().append("g").attr("transform", function (d) {
  ...
  d3.select(this).transition().duration(100).attr('r', 11).style('fill', '#BB1427'); // make tooltip visible

  div.transition().duration(100).style('opacity', 1);
  div.html(d.data.description).style('left', d3.event.pageX - 70 + 'px') // .style('right', d3.event.pageX + 'px')
  .style('top', d3.event.pageY + 15 + 'px');
}).on('mouseout', function (d) {
  d3.select(this).transition().duration(100).attr('r', 9).style('fill', '#B8A1A8'); // hide tooltip again
...
}).attr('height', 20) // .attr('fill', '#FFD67C')
.attr('fill', '#8ec1bc').style('opacity', 0.6); // add step labels
...
//# sourceMappingURL=/dendrogram.8f9fa2bc.js.map