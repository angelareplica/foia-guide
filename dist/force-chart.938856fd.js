...
var version = exports.version = "5.14.2";
...
var scripts = exports.scripts = { "pretest": "rimraf dist && mkdir dist && json2module package.json > dist/package.js && rollup -c", "test": "tape 'test/**/*-test.js'", "prepublishOnly": "yarn test", "postpublish": "git push && git push --tags && cd ../d3.github.com && git pull && cp ../d3/dist/d3.js d3.v5.js && cp ../d3/dist/d3.min.js d3.v5.min.js && git add d3.v5.js d3.v5.min.js && git commit -m \"d3 ${npm_package_version}\" && git push && cd - && cd ../d3-bower && git pull && cp ../d3/LICENSE ../d3/README.md ../d3/dist/d3.js ../d3/dist/d3.min.js . && git add -- LICENSE README.md d3.js d3.min.js && git commit -m \"${npm_package_version}\" && git tag -am \"${npm_package_version}\" v${npm_package_version} && git push && git push --tags && cd - && zip -j dist/d3.zip -- LICENSE README.md API.md CHANGES.md dist/d3.js dist/d3.min.js" };
var devDependencies = exports.devDependencies = { "json2module": "0.0", "rimraf": "2", "rollup": "1", "rollup-plugin-ascii": "0.0", "rollup-plugin-node-resolve": "3", "rollup-plugin-terser": "5", "tape": "4" };
...
var svg = d3.select('#force').append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('fill', 'lightgrey');

var radiusScale = d3.scaleSqrt().domain([75, 6300]).range([10, 80]);

var colorScale = d3.scaleOrdinal().range(['#8ec1bc', '#727FAD', '#8A496B']);

var div = d3.select('body').append('div').attr('class', 'tooltip force').style('opacity', 0);
...
var forceXSeparateRate = d3.forceX(function (d) {
...
var forceYSeparateRate = d3.forceY(function (d) {
  if (d.success_category === '0-25%') {
    return 220;
  } else if (d.success_category === '26-50%') {
    return 200;
...
d3.csv(require('./data/muckrock_top_100_agencies.csv')).then(ready).catch(function (err) {
  return console.log('Failed on', err);
});

function ready(datapoints) {
  var circles = svg.selectAll('.agency').data(datapoints).enter().append('circle').attr('class', 'agency').attr('r', function (d) {
    return radiusScale(d.number_requests);
  }).attr('fill', function (d) {
    return colorScale(d['jurisdiction-level']);
  }).attr('opacity', 0.8).on('mousemove', function (d) {
    div.html('Agency: '.bold() + d.name + '<br>' + 'Jurisdiction: '.bold() + d['jurisdiction name'] + '<br>' + 'Requests received: '.bold() + d.number_requests).style('left', d3.event.pageX + 'px').style('top', d3.event.pageY - 28 + 'px').style('display', 'block');
  }).on('mouseover', function (d) {
    d3.select(this).transition().duration(100).attr('opacity', 1).attr('stroke', '#FFD67C').attr('stroke-width', 2);
    div.transition().duration(200).style('opacity', 1);
  }).on('mouseout', function (d) {
    d3.select(this).transition().duration(100).attr('opacity', 0.7).attr('stroke', 'none');
    div.transition().duration(500).style('opacity', 0);
  });

  // group data for clusters
  var jurisNested = d3.nest().key(function (d) {
    return d['jurisdiction-level'];
  }).entries(datapoints);

  var successNested = d3.nest().key(function (d) {
    return d.success_category;
  }).entries(datapoints);

  svg.selectAll('label-jurisdiction').data(jurisNested).enter().append('text').text(function (d) {
    return d.key;
  }).attr('class', 'label-jurisdiction').attr('font-weight', 'bold').attr('x', function (d) {
    if (d.key === 'Federal') {
      return 110;
    } else if (d.key === 'State') {
      return 400;
  ...
  svg.selectAll('label-success').data(successNested).enter().append('text').text(function (d) {
    return d.key;
  }).attr('class', 'label-success').attr('font-weight', 'bold').attr('x', function (d) {
    if (d.key === '0-25%') {
      return 100;
    } else if (d.key === '26-50%') {
      return 420;
  ...
  d3.select('#jurisdiction-button').on('click', function () {
    svg.selectAll('.label-jurisdiction').transition().style('visibility', 'hidden');
    svg.selectAll('.agency').attr('fill', function (d) {
      return colorScale(d['jurisdiction-level']);
    });
  ...
  d3.select('#jurisdiction-button').on('click', function () {
    svg.selectAll('.label-jurisdiction').transition().style('fill', 'black').style('visibility', 'visible');

    svg.selectAll('.label-success').transition().style('fill', 'black').style('visibility', 'hidden');

    simulation.force('x', forceXSeparate).force('y', forceYSeparate).alphaTarget(0.7).restart();
  });

  d3.select('#success-rate-button').on('click', function () {
    svg.selectAll('.label-success').transition().style('fill', 'black').style('visibility', 'visible');

    svg.selectAll('.label-jurisdiction').transition().style('fill', 'black').style('visibility', 'hidden');

    simulation.force('x', forceXSeparateRate).force('y', forceYSeparateRate).alphaTarget(0.7).restart();
  });
...
},{"d3":"../node_modules/d3/index.js","./data/muckrock_top_100_agencies.csv":"data/muckrock_top_100_agencies.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","force-chart.js"], null)
//# sourceMappingURL=/force-chart.938856fd.map