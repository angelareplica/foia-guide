...
var yPositionScale = d3.scaleLinear().domain([300000, 900000]).range([height, 0]);
...
d3.csv(require('./data/annual_foia_data.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  }).attr('y', height).attr('width', xPositionScale.bandwidth()).attr('height', 0).attr('fill', '#8ec1bc').attr('opacity', '75%').on('mouseover', function (d) {
    // console.log(d)

    // make the bar chart highlight on mouseover
    d3.select(this).transition().duration(100).attr('opacity', '100%');

    div.transition().duration(100).style('opacity', 0.8);

    div.html(formatComma(+d['requests']) + ' requests filed').style('left', d3.event.pageX + 'px')
    // .style('right', d3.event.pageX + 'px')
    .style('top', d3.event.pageY - 28 + 'px');
  }).on('mouseout', function (d) {
    d3.select(this).transition().duration(100).attr('opacity', '75%');

    div.transition().duration(200).style('opacity', 0);
  });

  var yAxis = d3.axisLeft(yPositionScale).tickSize(5).ticks(6);

  svg.append('g').attr('class', 'axis y-axis').call(yAxis).style('visibility', 'hidden'); // set to hidden for scrollytelling

  // remove solid line on y-axis
  d3.select('.y-axis .domain').remove();

  var xAxis = d3.axisBottom(xPositionScale).tickFormat(d3.timeFormat('%Y')).tickSize(5);

  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis).style('visibility', 'hidden'); // set to hidden for scrollytelling

  d3.select('#intro').on('stepin', function () {
    // unhide axes
    svg.selectAll('.axis').transition().duration(500).style('visibility', 'visible');

    // add width of bars
    svg.selectAll('rect').transition().duration(500).attr('y', function (d) {
...
},{"d3":"../node_modules/d3/index.js","./data/annual_foia_data.csv":"data/annual_foia_data.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","annual-bar-chart.js"], null)
//# sourceMappingURL=/annual-bar-chart.04142981.map