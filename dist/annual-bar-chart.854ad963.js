...
module.exports = "/annual_foia_data.ce57c522.csv";
...
var svg = d3.select('#bar-chart').append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // .style('stroke', 'red')

var xPositionScale = d3.scaleBand().range([0, width]).padding(0.2);
var yPositionScale = d3.scaleLinear().domain([300000, 900000]).range([height, 0]); // add a div for the tooltip

var div = d3.select('body').append('div').attr('class', 'tooltip').style('opacity', 0); // add var for formatting tooltip numbers

var formatComma = d3.format(','); // var to turn year into datetime object
...
function ready(datapoints) {
  ...
  svg.selectAll('rect').data(datapoints).enter().append('rect').attr('x', function (d) {
    ...
    div.html(formatComma(+d['requests']) + ' requests filed').style('left', d3.event.pageX + 'px') // .style('right', d3.event.pageX + 'px')
  ...
  d3.select('#intro').on('stepin', function () {
    // unhide axes
    svg.selectAll('.axis').transition().duration(500).style('visibility', 'visible'); // add width of bars
...
//# sourceMappingURL=/annual-bar-chart.854ad963.js.map